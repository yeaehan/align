...
def inspect_and_extract(lif_path: str, output_dir: str):
    ...
    for i in range(reader.num_images):
        ...
        print(f"Scene {i+1}/{reader.num_images}: '{img.name}'")
        print(f"  Dimensions: X={img.dims.x}, Y={img.dims.y}, Z={img.dims.z}, Channels={img.channels}")
        
        # Experiment: Extracting all channels and applying Max Projection if 3D
        print(f"  -> Pipeline Simulation: Extracting all {img.channels} channels to float32 & applying MIP")
        
        # Loop through every channel in the current scene
        for c in range(img.channels):
            ...
            # Convert back to uint16 for saving (just like align.io.writer does)
